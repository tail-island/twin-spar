  ...
  (let [database (ts-database (jdbc/with-db-transaction [transaction database-spec]
    ...
    (testing "writing values"
      (let [database (-> database
        ...
        (is (= [(get-data (get-in database [:products (row-key 50)])) (get-data (get-in database [:products (row-key 51)]))]
               (sort-by :name (vals (:products (get-modified-rows database))))))
        (is (every? #(nil? (get (get-modified-rows database) %)) (keys (dissoc database-schema :products))))
        (is (every? #(nil? (get (get-deleted-rows  database) %)) (keys database-schema))))
      
      (is (= "p0" (get-in database [:products (row-key 50) :name])))

      (is (every? #(nil? (get (get-deleted-rows  database) %)) (keys database-schema)))
      (is (every? #(nil? (get (get-modified-rows database) %)) (keys database-schema)))
      ...
      (let [database (assoc-in database [:employees (row-key 31) :superior :name] "E0")]
        ...
        (is (= [(row-key 30)] (keys (:employees (get-modified-rows database))))))
      ...
      (let [database (assoc-in database [:employees (row-key 30) :organization :name] "O0")]
        ...
        (is (= [(row-key 10)] (keys (:organizations (get-modified-rows database)))))
        (is (nil? (:employees (get-modified-rows database)))))
      ...
      (let [database (-> database
        ...
        (is (= [(row-key 30)] (keys (:employees (get-deleted-rows database)))))
        (is (= [(row-key 10) (row-key 11)] (map first (sort-by (comp :name second) (:organizations (get-deleted-rows database))))))
        (is (every? #(nil? (get (get-deleted-rows  database) %)) (keys (dissoc database-schema :employees :organizations))))
        (is (every? #(nil? (get (get-modified-rows database) %)) (keys database-schema)))))
